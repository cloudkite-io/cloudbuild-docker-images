...
# required env vars MANAGED_IDENTITY_CLIENT_ID,SOURCE_DB_SERVER,SOURCE_DB_NAME,SOURCE_DB_RESOURCE_GROUP_NAME,SOURCE_DB_USER,SOURCE_DB_PASSWORD
# SOURCE_DB_PASSWORD,DESTINATION_DB_NAME,DESTINATION_DB_RESOURCE_GROUP_NAME,DESTINATION_DB_USER,DESTINATION_DB_PASSWORD

# unix timestamp
DATE=$(date +%s)

 . /azure/bin/activate && az login --identity --username ${MANAGED_IDENTITY_CLIENT_ID}
