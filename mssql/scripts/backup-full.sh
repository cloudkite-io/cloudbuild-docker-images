...
BACKUP_FILENAME="${DB_DATABASE}-full-$(date +%Y-%m-%d-%H-%M).bak"
S3_URL="s3://${S3_ENDPOINT}/backups/${DB_DATABASE}/full/${BACKUP_FILENAME}"
CREDENTIAL_NAME="s3://${S3_ENDPOINT}"

echo "Starting FULL backup for database [${DB_DATABASE}] to ${S3_URL}"
...
TO URL = '${S3_URL}'
WITH FORMAT, COMPRESSION, STATS = 10, MAXTRANSFERSIZE = 5242880;"
...
echo "FULL backup of [${DB_DATABASE}] completed successfully."
